...
class MyMplCanvas(FigureCanvas):
    ...
    def operate(self):
        ...
        if self.mode == 1:
            ...
            self.small_world_deffuant(self.number1, self.number2, self.steps, self.number5, self.number3,self.number4)
        elif self.mode==27:
            self.random_network_deffuant(self.number1, self.number2, self.steps, self.number5, self.number3,self.number4)
        elif self.mode==28:
            self.scale_free_deffuant(self.number1,self.number2,self.number3,self.number4,self.number5,self.steps)
    ...
    def grid_snow(self, n,step_times,a1,a2):
        ...
        if self.flag == 0:
            ...
            self.dll_41.run_over(n * n, step_times,c_double(a1),c_double(a2))  # 运行dll博弈程序
    ...
    def scale_free_prisoner(self, m1, m2, step_times, N1,a1,a2):
        ...
        if self.flag == 0:
            self.flag = 1
            xx = nx.draw_networkx_nodes(self.G, pos=self.pos_round, node_size=self.a,node_color=self.b,ax=self.axes)  # 生成节点
            xx.set_edgecolor('black')  # 设置节点边框
            nx.draw_networkx_edges(self.G, pos=self.pos_round, ax=self.axes, alpha=0.4)  # 生成边
        else:
            self.axes.hold(True)
    ...
    def scale_free_SI(self, m1, m2, a1,N1,step_times):
        ...
        if self.flag == 0:
            ...
            # self.flag=1
            print(3)
            self.dll_21.run_over(N1, step_times,c_double(a1))  # 执行博弈，时间步为1000
            # 获取数据，读取当前目录下的CSV文件
    ...
    def scale_free_deffuant(self, m1, m2, a1,a2,N1,step_times):
        self.mode = 28  # 网络类型
        self.n = N1  # 节点个数
        self.number1 = m1  # 初始节点数
        self.number2 = m2  # 新加入节点的度大小
        ...
        self.dll_21 = cdll.LoadLibrary('scale-free-deffuant.dll')
        node_change = [0] * N1
        change_flag = 0
        if self.step_time == step_times:
            self.stop_flag = 1
        ...
        if self.flag == 0:
            if self.flag_openFile == 0:
                self.dll_21.main(m1, m2, N1)  # dll初始化
                self.my_file = 'scale-free-deffuant-1.csv'
                print(2)
                self.df = pd.DataFrame(pd.read_csv(self.my_file))
            else:
                self.flag_openFile = 0
            ...
            print('complete')
            # 数据库
            self.dll_sql = sqlite3.connect("scale-free-deffuant.db")
            dff = pd.DataFrame(pd.read_csv('scale-free-deffuant-2.csv'))  # , sep='\t'
            cursor = self.dll_sql.cursor()  # 该例程创建一个 cursor，将在 Python 数据库编程中用到。
            self.dll_sql.row_factory = sqlite3.Row  # 可访问列信息
            dff.to_sql('some_table', self.dll_sql, if_exists='append')  # index=False
            self.dll_sql.execute("delete from some_table")
    ...
    def scale_free_election(self, m1, m2, step_times, N1):
        ...
        if self.flag == 0:
            ...
            # self.flag=1
            print(3)
            self.dll_21.run_over(N1, step_times)  # 执行博弈，时间步为1000
            # 获取数据，读取当前目录下的CSV文件
    ...
    def grid_SI(self, N,m1,step_times):
        ...
        if self.flag == 0:
            ...
            # self.flag=1
            print(3)
            self.dll_21.run_over(N,step_times,c_double(m1))  # 执行博弈，时间步为1000
            # 获取数据，读取当前目录下的CSV文件
            for i in range(self.n1):  # x坐标
                for j in range(self.n1):
                    self.pos_round[i * self.n1 + j][0] = j
    ...
    def grid_election(self, n, step_times):
        ...
        if self.flag == 0:
            ...
            self.ave_degree=self.sum/nx.number_of_nodes(self.G)
            print(3)
            self.dll_21.run_over(n, step_times)  # 执行博弈，时间步为1000
            # 获取数据，读取当前目录下的CSV文件
            for i in range(self.n1):  # x坐标
                for j in range(self.n1):
                    self.pos_round[i * self.n1 + j][0] = j
    ...
    def grid_deffuant(self, n, step_times,a1,a2):
        ...
        if self.flag == 0:
            ...
            # self.flag=1
            print(3)
            self.dll_21.run_over(n, step_times,c_double(a1),c_double(a2))  # 执行博弈，时间步为1000
            # 获取数据，读取当前目录下的CSV文件
            for i in range(self.n1):  # x坐标
                for j in range(self.n1):
                    self.pos_round[i * self.n1 + j][0] = j
    ...
    def random_network_SIS(self,number,p,step_times,N2,a1,a2):
        ...
        self.draw()
        self.axes.hold(False)
    def small_world_deffuant(self,number,p,step_times,N2,a1,a2):
        self.mode = 26  # 网络类型
        self.n = N2  # 节点个数
        self.number1 = number  # 邻居节点数
        self.number2 = p  # 概率
        ...
        self.steps = step_times  # 单步博弈次数
        self.dll_21 = cdll.LoadLibrary('small-world-deffuant.dll')
        ...
        if self.flag == 0:
            if self.flag_openFile == 0:
                self.dll_21.main(number,c_double(p),N2)  # dll初始化
                self.my_file = 'small-world-deffuant-1.csv'
                print(2)
                self.df = pd.DataFrame(pd.read_csv(self.my_file))
            else:
                self.flag_openFile = 0
            ...
            print('complete')
            # 数据库
            self.dll_sql = sqlite3.connect("small-world-deffuant.db")
            dff = pd.DataFrame(pd.read_csv('small-world-deffuant-2.csv'))  # , sep='\t'
            cursor = self.dll_sql.cursor()  # 该例程创建一个 cursor，将在 Python 数据库编程中用到。
            self.dll_sql.row_factory = sqlite3.Row  # 可访问列信息
            dff.to_sql('some_table', self.dll_sql, if_exists='append')  # index=False
            self.dll_sql.execute("delete from some_table")
    ...
    def random_network_deffuant(self,number,p,step_times,N2,a1,a2):
        self.mode = 27  # 网络类型
        self.n = N2  # 节点个数
        self.number1 = number  # 邻居节点数
        self.number2 = p  # 概率
        ...
        self.steps = step_times  # 单步博弈次数
        self.dll_21 = cdll.LoadLibrary('random-network-deffuant.dll')
        ...
        if self.flag == 0:
            if self.flag_openFile == 0:
                self.dll_21.main(number,c_double(p),N2)  # dll初始化
                self.my_file = 'random-network-deffuant-1.csv'
                print(2)
                self.df = pd.DataFrame(pd.read_csv(self.my_file))
            else:
                self.flag_openFile = 0
            ...
            print('complete')
            # 数据库
            self.dll_sql = sqlite3.connect("random-network-deffuant.db")
            dff = pd.DataFrame(pd.read_csv('random-network-deffuant-2.csv'))  # , sep='\t'
            cursor = self.dll_sql.cursor()  # 该例程创建一个 cursor，将在 Python 数据库编程中用到。
            self.dll_sql.row_factory = sqlite3.Row  # 可访问列信息
            dff.to_sql('some_table', self.dll_sql, if_exists='append')  # index=False
            self.dll_sql.execute("delete from some_table")
    ...
    def small_world_SI(self,number,p,step_times,N2,a1):
        ...
        if self.flag == 0:
            ...
            # self.flag=1
            print(3)
            self.dll_21.run_over(N2, step_times,c_double(a1))  # 执行博弈，时间步为1000
            # 获取数据，读取当前目录下的CSV文件
    ...
    def small_world_election(self, number, p, step_times, N2):
        ...
        if self.flag == 0:
            ...
            # self.flag=1
            print(3)
            self.dll_21.run_over(N2, step_times)  # 执行博弈，时间步为1000
            # 获取数据，读取当前目录下的CSV文件
